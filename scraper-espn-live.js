 ...
 * OPTIMIZADO: Reducción de tiempos de espera y mejor manejo de caché
 * -----------------------------------------------------------------------
 */
...
const CONFIG = {
    ...
    // 🔥 REDUCIDOS para mayor velocidad
    navigationTimeoutMs: 25000, // 25s (antes 60s)
    contentWaitMs: 1500,        // 1.5s (antes 4s)
    maxRetries: 2,              // 2 intentos (antes 3)
    retryDelayMs: 2000,         // 2s (antes 4s)
    outputDir: __dirname,
...
let navegadorCalentando = false;

async function obtenerNavegador() {
    // Si el navegador ya está calentado y conectado, reusarlo
    if (navegadorCompartido && navegadorCompartido.isConnected()) {
        return navegadorCompartido;
    }
    
    // Si el navegador existe pero no conecta, limpiar
    if (navegadorCompartido) {
        try { await navegadorCompartido.close(); } catch (e) {}
        navegadorCompartido = null;
    }
    
    // Si ya se está calentando, esperar a que termine
    if (navegadorCalentando) {
        log('⏳', 'Esperando a que el navegador termine de calentarse...');
        let esperas = 0;
        while (navegadorCalentando && esperas < 30) {
            await sleep(500);
            esperas++;
        }
        if (navegadorCompartido && navegadorCompartido.isConnected()) {
    ...
    navegadorCalentando = true;
    try {
        navegadorCompartido = await lanzarNavegador();
        navegadorCompartido.on('disconnected', () => {
            log('⚠️', 'El navegador compartido se desconectó');
            navegadorCompartido = null;
        });
        return navegadorCompartido;
    } finally {
        navegadorCalentando = false;
    }
}
...
async function navegarConReintentos(page, url) {
    ...
    for (let intento = 1; intento <= CONFIG.maxRetries; intento++) {
        try {
            ...
            // 🔥 Usar networkidle0 con timeout reducido
            await page.goto(url, {
                waitUntil: 'networkidle0',
                timeout: CONFIG.navigationTimeoutMs,
            });
            log('✅', `Página cargada (intento ${intento})`);
            return;
        } catch (err) {
            ultimoError = err;
...
async function scrapearPartidosEnVivo() {
    ...
    try {
        ...
        debug.etapa = 'navegador_obtenido';
        
        page = await prepararPagina(browser);
        debug.etapa = 'pagina_preparada';

        await navegarConReintentos(page, CONFIG.url);
        debug.etapa = 'navegacion_completa';

        // 🔥 REDUCIDO: En lugar de sleep fijo, esperar directamente al selector
        try {
            await page.waitForSelector('a[href*="/futbol/equipo/"]', { 
                timeout: 8000 // 8s máximo
            });
            log('✅', 'Tarjetas de partidos detectadas.');
            debug.tarjetasDetectadas = true;
        } catch {
            log('⚠️', 'No se detectaron tarjetas de partidos en 8s');
        ...
        debug.tiempoTotal = Date.now() - debug.tiempoInicio;

        // Logs
        log('🔍', `Estados detectados (${debugEstados.length}): ${JSON.stringify(debugEstados)}`);
        if (debugTarjetas.length > 0) {
            log('🔍', `Partidos en vivo encontrados: ${partidosEnVivo.length}`);
            debugTarjetas.forEach((d, i) => {
                console.log(`   [${i}] ${d.nombresEquipos.join(' vs ')}`);
            });
        }
